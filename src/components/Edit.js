...
class Edit extends Component {
    ...
    componentDidMount() {
        const ref = firebase.firestore().collection('article').doc(this.props.match.params.id);
        ref.get().then((doc) => {
            if (doc.exists) {
    ...
    onSubmit = (e) => {
        ...
        const updateRef = firebase.firestore().collection('article').doc(this.state.key);
        updateRef.set({
            title,
            body,
            author
